...
import { sendOtpOnEmail } from '../utils/sendOtpOnEmail.js'
...
import { generateAdminToken, generateOTPToken } from "../utils/generateToken.js";
...
// ======= adminLogin =======
export const adminLoginRequest = tryCatchWrapper(async (req, res) => {
  const { email, password } = req.body;
  const CheckUser = await AdminUser.findOne({
    $and: [{ email }, { role: 'admin' }, {password}],
  });
  if (!CheckUser) {
  ...
  const verifyPassword = CheckUser.comparePassword(password);
  if (!verifyPassword) {
  ...
  const token = generateOTPToken(CheckUser._id, otp);
...
export const adminLogin = tryCatchWrapper(async (req, res) => {
  ...
  // Verify the OTP token and decode the payload
  const decoded = jwt.verify(token, process.env.JWT_SECRECT_KEY);
  console.log(decoded);

  if (decoded.userId != CheckUser._id) {
    failedResponse(res, "Invalid token", 404);
    return
  }
  if (decoded.otp != otp) {
    failedResponse(res, "Invalid otp", 404);
    return
  }



  const token2 = generateAdminToken(CheckUser._id);
  successResponse(res, "login Successfull", token2);
});
// export const adminLogin = tryCatchWrapper(async (req, res) => {
...
export const changeAdminPassword = tryCatchWrapper(async (req, res) => {
  ...
  const CheckUser = await AdminUser.findById(userId);
  if (!CheckUser) {
    failedResponse(res, "User Not Found!", 404);
    return;
  }

  const isOldPasswordCorrect = CheckUser.comparePassword(password)
...
export const getAllUser = tryCatchWrapper(async (req, res) => {
  ...
  if (search) {
    query.$or = [
      ...
      { $expr: { $regexMatch: { input: { $toString: "$phone" }, regex: search } } }, // Search by phone number by converting it to a string
  ...
  try {
    ...
    successResponse(res, "AllUser Fetched!", {
      allUser,
      pagination: {
        currentPage: page,
        totalPages,
    ...
    failedResponse(res, "Error fetching users.");
  }
});
...
export const getAllBets = tryCatchWrapper(async (req, res) => {
  ...
  if (search) {
    query.$or = [
      { "userId.name": { $regex: search, $options: "i" } }, // Example search by user name
      { amount: { $regex: search, $options: "i" } } // Example search by amount if amount is a string
    ];
  }
  ...
  try {
    ...
    const totalCount = await oldBet.countDocuments(query);
    const totalPages = Math.ceil(totalCount / limit);

    successResponse(res, "All Bets Fetched Successfully!", {
      data,
      pagination: {
        currentPage: page,
        totalPages,
    ...
    failedResponse(res, "Error fetching bets.");
  }
});
...
export const getAllGameBets = tryCatchWrapper(async (req, res) => {
  ...
  try {
    ...
    successResponse(res, "Today's Bets Fetched Successfully!", {
      data,
      pagination: {
        currentPage: page,
        totalPages,
    ...
    failedResponse(res, "Error fetching game bets.");
  }
});
...
export const getDashboardData = tryCatchWrapper(async (req, res) => {
  ...
  const userQuery = user.find({ validuser: true });
  const userTodayQuery = user.find({
    createdAt: {
      $gte: startOfToday,
      $lte: now,
    },
  ...
  const rejectedUserQuery = user.find({ status: "Rejected" });

  const depositTotalQuery = transection.find({ type: "d" });
  const depositTodayQuery = transection.find({
    type: "d",
    createdAt: {
      $gte: startOfToday,
      $lte: now,
  ...
  const totalDepositAmountQuery = transection.aggregate([
    { $match: { status: "Approved", type: "d" } },
    { $group: { _id: null, totalAmount: { $sum: "$money" } } },
  ]);
  const todayDepositAmountQuery = transection.aggregate([
    {
      $match: {
        status: "Approved",
        type: "d",
  ...
  const withdrawTotalQuery = transection.find({ type: "w" });
  const withdrawTodayQuery = transection.find({
    type: "w",
    createdAt: {
      $gte: startOfToday,
      $lte: now,
  ...
  const totalWithdrawAmountQuery = transection.aggregate([
    { $match: { status: "Approved", type: "w" } },
    { $group: { _id: null, totalAmount: { $sum: "$money" } } },
  ]);
  const todayWithdrawAmountQuery = transection.aggregate([
    {
      $match: {
        status: "Approved",
        type: "w",
  ...
  const totalBonusQuery = transection.aggregate([
    { $match: { type: "b" } },
    { $group: { _id: null, totalAmount: { $sum: "$money" } } },
  ]);
  const todayBonusQuery = transection.aggregate([
    {
      $match: {
        type: "b",
        createdAt: {
  ...
  const totalBetsQuery = oldBet.find();
  const todayBetsQuery = oldBet.find({
    createdAt: {
      $gte: startOfToday,
      $lte: now,
    },
  });
  const totalBetAmountQuery = oldBet.aggregate([
    { $group: { _id: null, totalAmount: { $sum: "$betAmount" } } },
  ]);
  const todayBetAmountQuery = oldBet.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfToday,
  ...
  const totalWinAmountQuery = oldBet.aggregate([
    { $group: { _id: null, totalAmount: { $sum: "$winAmount" } } },
  ]);
  const todayWinAmountQuery = oldBet.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfToday,
  ...
  const totalGamesQuery = game.find({
    createdAt: {
      $gte: startOfToday,
      $lte: now,
    },
  ...
  const [
    ...
    userTodayQuery.countDocuments(),
    rejectedUserQuery.countDocuments(),
    depositTotalQuery.countDocuments(),
    depositTodayQuery.countDocuments(),
    totalDepositAmountQuery,
    todayDepositAmountQuery,
    withdrawTotalQuery.countDocuments(),
    withdrawTodayQuery.countDocuments(),
    totalWithdrawAmountQuery,
    todayWithdrawAmountQuery,
    totalBonusQuery,
    todayBonusQuery,
    totalBetsQuery.countDocuments(),
    todayBetsQuery.countDocuments(),
    totalBetAmountQuery,
    todayBetAmountQuery,
    totalWinAmountQuery,
    todayWinAmountQuery,
    totalGamesQuery.countDocuments(),
  ]);
  ...
  successResponse(res, "All Dashboard Data Fetched Successfully", data);
});
